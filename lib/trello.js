...
var trelloHelper = (function () {
  ...
  const listLookUp = getListLookUp(listData)

  let issueId = 0 // global counter for the "Issue ID" field

  return {
    boardName: () => boardData.name,
    cardToArray: () => cardData.flatMap(trelloHelper.cardToCsvLines),
    customFieldColNames: () => customFieldsData.map(field => CUSTOM_FIELD_PREFIX + field.name),
    columnFullOrder: () => [
      'id',
      'createdAt',
      'closed',
    ...
    cardToString (card) {
      ...
      const contentHandlers = {
        ...
        idMembers: (content) => [
          trelloHelper.processMemberIds(content, 'initials'),
          trelloHelper.processMemberIds(content, 'fullName')
        ],
        name: (content) => {
        ...
        badges: (content) => [trelloHelper.getVotesFromBadges(content)],
        attachments: (content) => [trelloHelper.getAttachmentURLs(content)]
      }
    ...
    processMemberIds (membersInCard = [], type) {
