var trelloHelper = (function() {
  ...
  return {
    ...
    customFieldPluginLookUpByName: function(){
      var pluginData = trelloHelper.customFieldPluginData();
      if(pluginData === null)
        return {};
      ...
      fieldsArray.forEach(function(field){
        lookUp[field.n] = field;
      });
      return lookUp;
    },

    customFieldPluginLookUpById: function(){
      var pluginData = trelloHelper.customFieldPluginData();
      if(pluginData === null)
        return {};
    ...
    getCustomFieldDropdownOptionValue: function(customFieldId, optionId){
      var optionValue = "";
      var dropdownOptionArray = trelloHelper.customFieldPluginLookUpById()[customFieldId]["o"];
      for(let i=0; i < dropdownOptionArray.length; i++){
        if(dropdownOptionArray[i]["id"] === optionId){
          optionValue = dropdownOptionArray[i]["value"];
          break;
        }
      }
      return optionValue;
    ...
    getCustomFieldDataValue: function(card, customFieldName){
      ...
      var customFieldId = trelloHelper.customFieldPluginLookUpByName()[customFieldName]["id"];
      ...
      pluginData.forEach(function(plugin){
        ...
        if(customFieldId in fields){
          // Type = 4 is a dropdown list, which data is in an array
          if(trelloHelper.customFieldPluginLookUpByName()[customFieldName]["t"] === 4){
            customFieldDataValue = 'list';
            customFieldDataValue = trelloHelper.getCustomFieldDropdownOptionValue(customFieldId, fields[customFieldId]);
          } else {
